...
describe("ReactUtil", function() {
  it("wrap the elements if there is more than one", function() {
  ...
  it("not wrap if not necessary", function() {
  ...
  it("properly handle undefined elements", function() {
    const elements = ReactUtil.wrapElements(undefined);

    expect(elements).toEqual(null);
  });

  it("not wrap elements if they are an array with a single item", function() {
    const elements = ReactUtil.wrapElements([<span key={0}>test</span>], "p");

    expect(TestUtils.isElementOfType(elements, "span")).toEqual(true);
  });

  it("wrap elements if they are an array with a single item when alwaysWrap is true", function() {
