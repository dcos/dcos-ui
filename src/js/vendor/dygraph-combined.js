 ...
 */
  ...
  DygraphLayout.prototype.setAnnotations = function(ann) {
    ...
    for (var i = 0; i < ann.length; i++) {
      ...
      if (
        ann[i].icon &&
        !(ann[i].hasOwnProperty("width") && ann[i].hasOwnProperty("height"))
      ) {
        console.error(
  ...
  DygraphLayout.prototype._evaluateLimits = function() {
    ...
    if (this.dygraph_.getOptionForAxis("logscale", "x")) {
      this._xAxis.xlogrange =
        Dygraph.log10(this._xAxis.maxval) - Dygraph.log10(this._xAxis.minval);
      this._xAxis.xlogscale = this._xAxis.xlogrange !== 0
        ? 1.0 / this._xAxis.xlogrange
        : 1.0;
    }
    for (var i = 0; i < this.yAxes_.length; i++) {
      ...
      if (this.dygraph_.getOption("logscale")) {
        axis.ylogrange =
          Dygraph.log10(axis.maxyval) - Dygraph.log10(axis.minyval);
  ...
  DygraphLayout.calcXNormal_ = function(value, xAxis, logscale) {
    if (logscale) {
      return (
        (Dygraph.log10(value) - Dygraph.log10(xAxis.minval)) * xAxis.xlogscale
      );
    } else {
 ...
 */
  DygraphLayout.calcYNormal_ = function(axis, value, logscale) {
    if (logscale) {
      var x =
        1.0 -
        (Dygraph.log10(value) - Dygraph.log10(axis.minyval)) * axis.ylogscale;
    ...
    if (kmb || kmg2) {
      ...
      if (kmg2) {
        ...
        if (x_parts.length === 2 && x_parts[1] >= 3 && x_parts[1] <= 24) {
          if (x_parts[1] % 3 > 0) {
            label = Dygraph.round_(
              x_parts[0] / Dygraph.pow(10, x_parts[1] % 3),
              digits
            );
          } else {
  ...
  Dygraph.dateAxisLabelFormatter = function(date, granularity, opts) {
    ...
    if (granularity >= Dygraph.DECADAL) {
      ...
      if (frac === 0 || granularity >= Dygraph.DAILY) {
        // e.g. '21 Jan' (%d%b)
        return (
          Dygraph.zeropad(day) + "&#160;" + Dygraph.SHORT_MONTH_NAMES_[month]
        );
      } else {
 ...
 */
  Dygraph.prototype.createInterface_ = function() {
    ...
    this.mouseOutHandler_ = function(e) {
      ...
      if (
        Dygraph.isNodeContainedBy(target, dygraph.graphDiv) &&
    ...
    this.doAnimatedZoom(old_window, new_window, null, null, function() {
      if (that.getFunctionOption("zoomCallback")) {
        that
          ...
          .call(that, minDate, maxDate, that.yAxisRanges());
      }
    });
  };
  ...
  Dygraph.prototype.doZoomY_ = function(lowY, highY) {
    ...
    this.doAnimatedZoom(null, null, oldValueRanges, newValueRanges, function() {
      if (that.getFunctionOption("zoomCallback")) {
        ...
        that
          .getFunctionOption("zoomCallback")
          .call(that, xRange[0], xRange[1], that.yAxisRanges());
      }
    });
  };
 ...
 */
  Dygraph.prototype.resetZoom = function() {
    ...
    for (var i = 0; i < this.axes_.length; i++) {
      if (
        typeof this.axes_[i].valueWindow !== "undefined" &&
        this.axes_[i].valueWindow !== null
      ) {
        dirty = true;
        dirtyY = true;
      }
    ...
    if (dirty) {
      ...
      if (!this.getBooleanOption("animatedZooms")) {
        ...
        if (this.getFunctionOption("zoomCallback")) {
          this.getFunctionOption("zoomCallback").call(
            this,
            minDate,
            maxDate,
            this.yAxisRanges()
      ...
      if (dirtyY) {
        ...
        for (i = 0; i < this.axes_.length; i++) {
          ...
          newValueRanges.push(
            axis.valueRange !== null && axis.valueRange !== undefined
              ? axis.valueRange
              : axis.extremeRange
          );
        }
 ...
 */
  Dygraph.prototype.renderGraph_ = function(is_initial_draw) {
    ...
    if (this.getFunctionOption("underlayCallback")) {
      ...
      this.getFunctionOption("underlayCallback").call(
        this,
        this.hidden_ctx_,
 ...
 */
  Dygraph.prototype.computeYAxes_ = function() {
    ...
    if (
      ...
      this.user_attrs_.hasOwnProperty("valueRange") === false
    ) {
      valueWindows = [];
      ...
      if (this.fractions_) {
        for (j = 1; j < inFields.length; j++) {
          ...
          if (vals.length != 2) {
            ...
            fields[j] = [
              Dygraph.parseFloat_(vals[0], i, line),
        ...
        for (j = 1; j < inFields.length; j += 2) {
          fields[(j + 1) / 2] = [
            Dygraph.parseFloat_(inFields[j], i, line),
  ...
  Dygraph.prototype.parseDataTable_ = function(data) {
    ...
    for (i = 1; i < cols; i++) {
      ...
      if (type == "number") {
        ...
        type == "string" && this.getBooleanOption("displayAnnotations")
      ) {
 ...
 */
  Dygraph.prototype.start_ = function() {
    ...
    if (Dygraph.isArrayLike(data)) {
      ...
      typeof data == "object" && typeof data.getColumnRange == "function"
    ) {
      ...
      if (line_delimiter) {
        ...
        req.onreadystatechange = function() {
          if (req.readyState == 4) {
            if (
              req.status === 200 || req.status === 0 // Normal http
            ) {
 ...
 */
  Dygraph.addAnnotationRule = function() {
    ...
    console.warn(
      "Unable to add default annotation CSS rule; display may be off."
    );
  };
    ...
    for (i = 0; i < g.axes_.length; i++) {
      ...
      if (logscale) {
        ...
        axis_data.dragValueRange =
          Dygraph.log10(yRange[1]) - Dygraph.log10(yRange[0]);
      } else {
 ...
 */
  Dygraph.Interaction.movePan = function(event, g, context) {
    ...
    if (g.getOptionForAxis("logscale", "x")) {
      g.dateWindow_ = [
        Math.pow(Dygraph.LOG_SCALE, minDate),
    ...
    if (context.is2DPan) {
      ...
      for (var i = 0; i < g.axes_.length; i++) {
        ...
        if (g.attributes_.getForAxis("logscale", i)) {
          axis.valueWindow = [
            Math.pow(Dygraph.LOG_SCALE, minValue),
 ...
 */
  Dygraph.Interaction.moveZoom = function(event, g, context) {
    ...
    context.dragDirection = xDelta < yDelta / 2
      ? Dygraph.VERTICAL
      : Dygraph.HORIZONTAL;
 ...
 */
  Dygraph.Interaction.treatMouseOpAsClick = function(g, event, context) {
    ...
    for (var i = 0; i < g.selPoints_.length; i++) {
      ...
      if (
        !isNaN(distance) && (closestIdx == -1 || distance < closestDistance)
      ) {
 ...
 */
  Dygraph.Interaction.endZoom = function(event, g, context) {
    ...
    if (
      context.regionWidth >= 10 && context.dragDirection == Dygraph.HORIZONTAL
    ) {
      ...
      context.regionHeight >= 10 && context.dragDirection == Dygraph.VERTICAL
    ) {
 ...
 */
  Dygraph.Interaction.moveTouch = function(event, g, context) {
    ...
    if (didZoom && touches.length > 1 && g.getFunctionOption("zoomCallback")) {
      ...
      g
        .getFunctionOption("zoomCallback")
        .call(g, viewWindow[0], viewWindow[1], g.yAxisRanges());
    }
  };
  ...
  Dygraph.numericTicks = function(a, b, pixels, opts, dygraph, vals) {
    ...
    if (vals) {
      ...
      if (opts("logscale")) {
        ...
        var minIdx = Dygraph.binarySearch(
          a,
          Dygraph.PREFERRED_LOG_TICK_VALUES,
          1
        );
        var maxIdx = Dygraph.binarySearch(
          b,
          Dygraph.PREFERRED_LOG_TICK_VALUES,
          -1
        );
    ...
    if (userLabelsDiv && null !== userLabelsDiv) {
      ...
      for (var name in messagestyle) {
        ...
        try {
          ...
          console.warn(
            "You are using unsupported css properties for your " +
  ...
  legend.prototype.select = function(e) {
    ...
    if (legendMode === "follow") {
      ...
      if (leftLegend + labelsDivWidth + 1 > area.w) {
        leftLegend =
          leftLegend - 2 * 50 - labelsDivWidth - (yAxisLabelWidth - area.x);
      }
  ...
  generateLegendDashHTML = function(strokePattern, color, oneEmWidth) {
    ...
    for (j = 0; j < loop; j++) {
      for (i = 0; i < segmentLoop; i += 2) {
        ...
        dash +=
          '<div style="display: inline-block; position: relative; ' +
          "bottom: .5ex; margin-right: " +
          marginRight +
          "em; padding-left: " +
          paddingLeft +
          "em; height: 1px; border-bottom: 2px solid " +
          color +
          ';"></div>';
      }
    }
 ...
 */
  rangeSelector.prototype.drawMiniPlot_ = function() {
    ...
    for (var i = 0; i < combinedSeriesData.data.length; i++) {
      ...
      var x = dataPoint[0] !== null
        ? (dataPoint[0] - xExtremes[0]) * xFact
        : NaN;
      var y = dataPoint[1] !== null
 ...
 */
  rangeSelector.prototype.getZoomHandleStatus_ = function() {
    ...
    return {
      ...
      isZoomed: leftHandlePos - 1 > this.canvasRect_.x ||
 ...
 */
  ...
  BarsHandler.prototype.onLineEvaluated = function(points, axis, logscale) {
    ...
    for (var j = 0; j < points.length; j++) {
      ...
      point = points[j];
      point.y_top = DygraphLayout.calcYNormal_(
        axis,
        point.yval_minus,
        logscale
      );
      point.y_bottom = DygraphLayout.calcYNormal_(
        axis,
        point.yval_plus,
        logscale
      );
    ...
    for (i = 0; i < originalData.length; i++) {
      ...
      if (wilsonInterval) {
        ...
        if (den) {
          ...
          var pm =
            sigma * Math.sqrt(p * (1 - p) / n + sigma * sigma / (4 * n * n));
