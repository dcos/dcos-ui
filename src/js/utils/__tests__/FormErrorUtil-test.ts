...
describe("FormErrorUtil", () => {
  describe("#getTopLevelTabErrors", () => {
    ...
    it("does not return an object containing errors with the same message", () => {
      expect(Object.keys(topLevelErrors).length).toEqual(3);
    });
    it("returns an object with the same keys as the object passed to the second parameter", () => {
      expect(Object.keys(topLevelErrors)).toEqual(Object.keys(tabRegexes));
    });
    it("maps error paths based on the regexes in the second parameter", () => {
  ...
  describe("#getContainerTabErrors", () => {
    const baseErrors = [
      {
        path: ["foo"],
        message: "Foo must be defined",
