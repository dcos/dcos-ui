 ...
 */
export function getUrl(nodeID, options, isStream = true, endpoint = "") {
  ...
  const idArray = ["framework", "executor", "container"].reduce((memo, key) => {
    const id = `${key}ID`;
 ...
 */
export function accumulatedThrottle(callback, wait) {
  let timeoutID;
  let accumulatedData = [];
