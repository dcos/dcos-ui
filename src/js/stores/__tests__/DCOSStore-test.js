...
describe("DCOSStore", function() {
  ...
  describe("#constructor", function() {
    it("expose an empty deployments list", function() {
  ...
  describe("#onMarathonDeploymentsChange", function() {
    ...
    describe("when the groups endpoint is not populated", function() {
      ...
      it("not update the deployments list", function() {
        expect(DCOSStore.deploymentsList.getItems().length).toEqual(0);
      });

      it("not update the notification store", function() {
        expect(NotificationStore.addNotification).not.toHaveBeenCalled();
      });

      it("be called as soon as groups endpoint data arrives", function() {
    ...
    describe("when the deployments endpoint references stale services", function() {
      ...
      it("trim stale services", function() {
        const deployment = DCOSStore.deploymentsList.last();
        const services = deployment.getAffectedServices();
  ...
  describe("#onMarathonGroupsChange", function() {
    ...
    it("update the service tree", function() {
    ...
    it("replace old Marathon data", function() {
      MarathonStore.__setKeyResponse(
        "groups",
        new ServiceTree({
          items: [
    ...
    it("merge (matching by id) summary data", function() {
      MarathonStore.__setKeyResponse(
        "groups",
        new ServiceTree({
          items: [
    ...
    it("not merge summary data if it doesn't find a matching id", function() {
      MarathonStore.__setKeyResponse(
        "groups",
        new ServiceTree({
          items: [
    ...
    it("not include _itemData", function() {
      MarathonStore.__setKeyResponse(
        "groups",
        new ServiceTree({
          items: [
  ...
  describe("#onMarathonQueueChange", function() {
    ...
    it("correctly convert running apps from waiting state", function() {
      expect(DCOSStore.serviceTree.getItems()[0].getStatus()).toEqual(
        "Waiting"
      );
      DCOSStore.onMarathonQueueChange([]);
  ...
  describe("#processMarathonServiceVersions", function() {
    ...
    it("merge existing version data", function() {
      DCOSStore.onMarathonServiceVersionChange({
        serviceID: "/beta",
  ...
  describe("#onMesosSummaryChange", function() {
    ...
    it("replace old summary data", function() {
      MesosSummaryStore.__setKeyResponse(
        "states",
        new SummaryList({
          items: [
    ...
    it("not merge Marathon data if it doesn't find a matching id", function() {
      MesosSummaryStore.__setKeyResponse(
        "states",
        new SummaryList({
          items: [
