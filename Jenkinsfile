...
pipeline {
  ...
  stages {
    ...
    stage('Upload Build') {
      when {
        expression { env.BRANCH_NAME == 'master' || env.BRANCH_NAME =~ /^v?(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(-(0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*)(\.(0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*))*)?(\+[0-9a-zA-Z-]+(\.[0-9a-zA-Z-]+)*)?/ || env.BRANCH_NAME == 'bill/feat/dcos-ui-cd' || env.BRANCH_NAME == 'test-dcos-ui-cd2' }
      }
