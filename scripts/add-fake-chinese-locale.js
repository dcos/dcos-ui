...
if (fs.existsSync(path.resolve(__dirname, "../locale/zh/messages.json"))) {
  console.log("locale/zh/messages already exists!");
  process.exit(1);
}

execSync("npx lingui add-locale zh");
execSync("npm run lingui-extract-with-plugins");

const enCatalog = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "../locale/en/messages.json"), "utf8")
);

Object.keys(enCatalog).forEach(key => {
  // Render 1 chinese character for each 5 english characters
  let trans = "";
  for (let x = 0; x < parseInt(key.length / 5, 10) + 1; x++) {
    let random;
    while (!random || names[random] === " ") {
      random = Math.floor(Math.random() * Math.floor(names.length - 1));
    }
    trans += names[random] + " ";
  }

  enCatalog[key]["translation"] = trans.trim();
});

fs.writeFile(
  path.resolve(__dirname, "../locale/zh/messages.json"),
  ...
  err => {
    ...
    console.log(JSON.stringify(enCatalog));
    console.log(
      `writing to '${path.resolve(__dirname, "../locale/zh/messages.json")}'`
    );
  }
);
