...
describe('Volumes', function () {
  describe('#JSONReducer', function () {
    ...
    it('should have an array with one object containing only a name',
        function () {
          let batch = new Batch();
          batch = batch.add(new Transaction(['volumeMounts'], 0, ADD_ITEM));
          batch =
              batch.add(new Transaction(['volumeMounts', 0, 'name'], 'foo'));
          batch =
  ...
  describe('#FormReducer', function () {
    it('should have an array with one object', function () {
      ...
      expect(batch.reduce(VolumeMounts.FormReducer.bind({}), []))
      .toEqual([{mountPath: []}]);
    });
    ...
    it('should have to items with names', function () {
      ...
      expect(batch.reduce(VolumeMounts.FormReducer.bind({}), []))
      .toEqual([{name: 'foo', mountPath: []}, {name: 'bar', mountPath: []}]);
    });
