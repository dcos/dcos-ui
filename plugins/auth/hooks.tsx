...
module.exports = {
  ...
  initialize() {
    this.filters.forEach(({ id, priority }) => {
      SDK.getSDK().Hooks.addFilter(id, this[id].bind(this), priority);
    });
    this.actions.forEach(({ id, priority }) => {
      SDK.getSDK().Hooks.addAction(id, this[id].bind(this), priority);
    });
    this.configure(SDK.getSDK().config);

    this.unregisterMesosDeniedAccess = this.unregisterMesosDeniedAccess.bind(
      this
    );

    this.registerMesosDeniedAccess = this.registerMesosDeniedAccess.bind(this);
    ...
    this.arePluginsConfigured = false;
  ...
  authorizedToAccessLocation(location) {
    ...
    return SIDEBAR_MENU_MAP[currentRoutePath].some(api =>
      SDK.getSDK().Hooks.applyFilter("hasCapability", false, api)
    );
  },
  ...
  fetchPermissionsForUserID(userID, callback) {
    ...
    ACLAuthenticatedUserStore.once(ACL_AUTH_USER_PERMISSIONS_CHANGED, () => {
      callback();
      SDK.getSDK().Hooks.doAction("userCapabilitiesFetched");
    });
  ...
  handleRouteChange({ pathname }) {
    ...
    if (this.arePluginsConfigured === false) {
      return;
    }
    ...
    const isAuthorized = SDK.getSDK().Hooks.applyFilter(
      "hasAuthorization",
      false,
      pathname
    );
  ...
  filterViewableItems(constraintMap, filterableArray) {
    const hasCapability = SDK.getSDK().Hooks.applyFilter.bind(
      SDK.getSDK().Hooks,
      "hasCapability",
      false
    );
  ...
  sidebarNavigation(menuItems) {
    const filteredMenu = this.filterViewableItems(SIDEBAR_MENU_MAP, menuItems);
